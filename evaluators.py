...
class SuccessiveHalvingEvaluator:
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompt=None, attribute_cache=None, rounds=40,
        ...
        for r in range(0, math.ceil(math.log2(n))):
            ...
            while True:
                try:
                    scores = scorer(predictor, S, sample, pred_prompt, attribute_cache, max_threads=max_threads)
                    break
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompt=None, attribute_cache=None, rounds=40,
        ...
        with tqdm(total=len(idx2prompt), desc='sr') as pbar:
            while True:
                ...
                while True:
                    try:
                        scores = scorer(predictor, selected_prompts, selected_data,
                                        pred_prompt, attribute_cache, max_threads=max_threads)
                        break
                    except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError,
                            urllib3.exceptions.MaxRetryError):
                        pass
...
class UCBBanditEvaluator:
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompt=None, attribute_cache=None, rounds=40,
                 num_prompts_per_round=10, samples_per_eval=5, max_threads=1, verbose=True):
        assert self.config['evaluator'] in {'ucb', 'ucb-e'}, f'unk evaluator: {self.config["evaluator"]}'
        bandit_algo = UCBBandits(len(prompts), num_samples=samples_per_eval,
                                 mode=self.config['evaluator'],
                                 c=self.config['c'])

        def data_sampler(l):
            return random.sample(l, min(len(l), samples_per_eval))
        ...
        for ri in tqdm(range(rounds), desc=f'Evaluating {len(prompts)} prompts'):
            ...
            while True:
                try:
                    scores = scorer(predictor, sampled_prompts, sampled_data,
                                    pred_prompt, attribute_cache, max_threads=max_threads)
                    break
                except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError,
                        urllib3.exceptions.MaxRetryError):
...
class BruteForceEvaluator:
    ...
    def __call__(self, prompts, exs, predictor, scorer, pred_prompt=None, attribute_cache=None, rounds=40,
                 num_prompts_per_round=10, c=2.0, samples_per_eval=5, max_threads=1, verbose=True):
        sample_size = min(len(exs), int(self.config['eval_budget'] / len(prompts)))
        eval_exs = random.sample(exs, sample_size)

        while True:
            try:
                scores = scorer(predictor, prompts, eval_exs, pred_prompt, attribute_cache, max_threads=max_threads)
                break
            except (concurrent.futures.process.BrokenProcessPool, requests.exceptions.SSLError,
                    urllib3.exceptions.MaxRetryError):
                pass
